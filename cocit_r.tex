...
Key to Uzzi {\em et al.}, however, is their random graph model and its underlying assumptions. A careful examination of the model (as defined by the Monte Carlo simulation) reveals that random substitutions of references used to generate expected values are equiprobable, which means that a reference in an article can be substituted by any other reference randomly chosen from those published in the same year, irrespective of disciplinary focus and citation count. 
...
Lastly,  as we will show here, while the citation switching algorithm used in Uzzi  {\em et al.} preserves the number of publications and the number of references in each publication, it does not preserve the disciplinary proportions of cited references within the network.
