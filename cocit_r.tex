...
Observed frequencies of these journal pairs across the dataset were computed and compared to expected values generated by Monte Carlo simulations that shuffled cited references in the context of a random graph model, allowing substitutions of references by any other reference published in the same year and appearing in WoS.
By normalizing the differences in these frequencies, journal pairs were assigned \emph{z-scores} (Materials and Methods). 
...
The finding of considerable interest was that HCHN articles were twice as likely to be found in highly cited papers~\citep{uzzi_atypical_2013} suggesting that novel ideas flavoring a base of conventional thought were a recipe for impact. 
The authors examined subsets of publications grouped by disciplined and asserted universality of this feature.

Key to the Uzzi {\em et al.} study, however, is the random graph approach and the mechanism of citation shuffling employed. 
Their random graph model produces
 ...
 However, the approach also assumes that all substitutions are equiprobable (i.e., a substitution of a reference in physics by a reference in musicology has the same probability under their model as a substitution of the physics reference by another physics reference).
...
% reference, which also is inconsistent with author behavior. 
 ...
 In addition, under the Uzzi {\em et al.} random model,  a reference cited over 100 times in a given year is selected with the same probability as a reference cited only once. 
 Accordingly, the expected value calculations generated by simulations and used in normalizing observed journal-pair-frequencies can be reasonably questioned on grounds of model mis-specification. 
...
Despite different methods used to generate expected values, both studies generated expected journal-pair frequencies without disciplinary considerations~\citep{uzzi_atypical_2013,boyack_vs_uzzi_2014}, and only after this were subsets of publications  examined for disciplinary effects. 
