...
%due to considering disciplinarily irrelevant references as a concern when disciplinary subsets are analyzed using a random graph model that generates expected co-citation frequencies from the WoS superset. 
%A symptom of model misspecification is that random sampling of citations associated with an article do not match with the distribution of citations across the disciplines observed in articles from that discipline. 
...
\textcolor{blue}{Would be nice to know the proportion of publications in each of the five major subject areas.}
...
disciplinary subnetwork) consistently have a lower K-L divergence compared to simulations that all replacements uniformly  from the WoS superset. 
...
The greatest hit rates for the applied physics data were often in the HNLC category and otherwise in the HNHC category (Figure \ref{fig:Fig2}).
We conclude that Uzzi et al.'s finding of high hit rates in the HNHC category does not hold generally for disciplinary-based datasets and that novel citation patterns are not always indicative of impactful research, as was the case with immunology. 
Furthermore, the categories displaying the greatest hit rate vary with parameter settings and with the year. 
