...
We hypothesized that assessing articles' novelty, conventionality, and impact on datasets focused in disciplinary areas would reduce model mis-specification and better simulate citation practice, in turn, leading to different and more relevant conclusions. Accordingly, we used keyword searches of the scientific literature to create three citation networks themed around academic disciplines. Within these disciplinary frameworks, we calculated expected journal pair frequencies using a random graph model that accounted for existing citation frequencies and an efficient Monte Carlo simulation algorithm that permitted a substantially greater number of simulations than do previous studies. We found our model did, in fact, preserve the citation patterns found in the literature whereas the approach of Uzzi et al. does not. The varying citation patterns when sampling from a disciplinary-focused dataset versus a broader dataset due, was a cause of journal pair z-scores changing signs in 28.6\% of instances: the effect of any one of these journal pairs on an article's novelty or conventiality is contradictory between broad and, more narrow disciplinary datasets. We contend that the interpretation due to a disciplinary dataset, which preserved citation patterns, is more appropriate and an improvement on current methods. Furthermore, our analysis of these semantically-themed citation networks challenges the universality of articles in the HCHN category having the highest probability of being hits as found in \cite{uzzi_atypical_2013}. For example, while HC remains highly correlated with hit articles in immunology and metabolism research, HN is not. In addition, while HN is highly correlated with hit articles in applied physics, HC is not.  Furthermore, we found that the categories demonstrating the highest percentage of hits is not robust with respect to varying parameters that define hit articles and the threshold for highly novel citation patterns. 
