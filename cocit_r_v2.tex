...
\emph{The revised manuscript is much improved, and the specific comments to both reviews were very helpful. Overall, the authors have addressed all of the issues I’ve raised satisfactorily. The paper is important as a critique to Uzzi.
The paper does not claim to present an alternative (better) way to identify innovative papers.  But I was hoping that it could- and agree with the editor that the additional requirement (of comparing evaluations from both approaches) is not needed.
...
\begin{itemize}
\item Conventionality and Innovativeness are part of the framework developed by Uzzi. We neither challenge nor embrace the framework but simply use it a read out. The reviewer is holding us responsible for Uzzi's construct.
\item We provide quantitative data addressing the reference count issue.
\end{itemize}
...
An irresponsible choice of words- `I do not think this paper can get away with it...' We are not trying to get away with anything!  Perhaps the reviewer confuses her/his own clumsy sophistry with our presentation? If so, it is most regrettable and we will strive to  ensure a distinction. Secondly, the reviewer's critique suffers from being somewhat incoherent. Even so....
\begin{itemize}
\item An illustration has been provided
\item Yes, a difference between bibliometrics and mathematics may be that bibliometrics papers cite mathematics papers more often than the other way around. The local model accounts for this but the global one does not
\item The relationship between novelty and conventionality is not a ground truth, we have never claimed it is. This framework was developed by Uzzi et al. (2013) and we have used it to enable \emph{ceteris paribus} comparisons
\end{itemize}
